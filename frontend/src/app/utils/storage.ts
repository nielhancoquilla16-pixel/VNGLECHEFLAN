...
export interface StaffRestockAlert {
  id: string;
  createdAt: string;
  createdBy: string;
  createdByName: string;
  note?: string;
  lowItems: Array<{ id: string; name: string; inventory: number; unit?: string }>;
  status: 'unread' | 'read';
}
...
const initializeData = () => {
  ...
  const mergedUsers = defaultUsers.map((defaultUser) => {
    const existing = filteredExisting.find((u) => u.email && u.email === defaultUser.email);
  ...
  const extraUsers = filteredExisting.filter(
    (u) => !mergedUsers.some((m) => m.email && m.email === u.email)
  );

  safeStorage.setItem('users', JSON.stringify([...mergedUsers, ...extraUsers]));

  if (!safeStorage.getItem('pendingPasswordResets')) {
    safeStorage.setItem('pendingPasswordResets', JSON.stringify([]));
  }
...
interface Revenue {
  ...
  yearlyHistory?: Array<{ yearStart: string; total: number }>;
}
...
function normalizeRevenue(revenue: Revenue) {
  ...
  const currentYearStart = new Date(now.getFullYear(), 0, 1, 0, 0, 0, 0).toISOString();
  ...
  revenue.yearly = revenue.yearly ?? 0;
  revenue.lastYearlyStart = revenue.lastYearlyStart ?? currentYearStart;
  revenue.yearlyHistory = revenue.yearlyHistory ?? [];

  if (revenue.lastYearlyStart !== currentYearStart) {
    revenue.yearlyHistory.push({ yearStart: revenue.lastYearlyStart, total: revenue.yearly });
    // keep last 5 years
    if (revenue.yearlyHistory.length > 5) revenue.yearlyHistory.shift();
    revenue.yearly = 0;
    revenue.lastYearlyStart = currentYearStart;
  }
...
export const storage = {
  ...
  getUserByIdentifier: (identifier: string): User | null => {
    const users: User[] = parseJSON(safeStorage.getItem('users'), []);
    return users.find(u => u.email === identifier || u.phone === identifier || u.username === identifier) || null;
  },
  ...
  savePendingRegistration: (pending: PendingRegistration) => {
    ...
    const filtered = pendingList.filter((p) =>
      (pending.email && p.email !== pending.email) || (pending.phone && p.phone !== pending.phone)
    );
    safeStorage.setItem('pendingRegistrations', JSON.stringify([...filtered, pending]));
  },

  getPendingRegistration: (identifier: string): PendingRegistration | null => {
    const pendingList: PendingRegistration[] = parseJSON(safeStorage.getItem('pendingRegistrations'), []);
    return pendingList.find((p) => p.email === identifier || p.phone === identifier) || null;
  },

  removePendingRegistration: (identifier: string) => {
    const pendingList: PendingRegistration[] = parseJSON(safeStorage.getItem('pendingRegistrations'), []);
    const filtered = pendingList.filter((p) => p.email !== identifier && p.phone !== identifier);
    safeStorage.setItem('pendingRegistrations', JSON.stringify(filtered));
  },

  savePasswordReset: (reset: PendingPasswordReset) => {
    const list: PendingPasswordReset[] = parseJSON(safeStorage.getItem('pendingPasswordResets'), []);
    const filtered = list.filter((r) =>
      (reset.email && r.email !== reset.email) || (reset.phone && r.phone !== reset.phone)
    );
    safeStorage.setItem('pendingPasswordResets', JSON.stringify([...filtered, reset]));
  },

  getPasswordReset: (identifier: string): PendingPasswordReset | null => {
    const list: PendingPasswordReset[] = parseJSON(safeStorage.getItem('pendingPasswordResets'), []);
    return list.find((r) => r.email === identifier || r.phone === identifier) || null;
  },

  removePasswordReset: (identifier: string) => {
    const list: PendingPasswordReset[] = parseJSON(safeStorage.getItem('pendingPasswordResets'), []);
    const filtered = list.filter((r) => r.email !== identifier && r.phone !== identifier);
    safeStorage.setItem('pendingPasswordResets', JSON.stringify(filtered));
  },
  ...
  saveProduct: (product: Product) => {
    ...
    safeStorage.setItem('products', JSON.stringify(products));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:products-updated'));
    } catch {
      // ignore
    }
  },
  ...
  // Online orders helpers
  getOnlineOrders: (): Order[] => {
    return storage.getOrders().filter((o) => o.orderType === 'online');
  },
  hasOnlineOrders: (): boolean => {
    return storage.getOnlineOrders().length > 0;
  },
  ...
  saveOrder: (order: Order) => {
    ...
    const applyPaidSideEffects = (paidOrder: Order) => {
      storage.addRevenue(paidOrder.total);
      storage.updatePurchaseCounts(paidOrder.items);

      // auto-deduct inventory on paid orders
      const products = storage.getProducts();
      let changed = false;
      paidOrder.items.forEach((item) => {
        const p = products.find((x) => x.id === item.productId);
        if (!p) return;
        const next = Math.max(0, Number(p.inventory ?? 0) - Number(item.quantity ?? 0));
        if (next !== p.inventory) {
          p.inventory = next;
          changed = true;
        }
    ...
    // Notify any listeners (same tab included) that orders were updated.
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:orders-updated'));
    } catch {
      // ignore in environments without window (SSR / tests)
    }
  ...
  clearOrderHistory: () => {
    const orders = storage.getOrders();
    const filtered = orders.filter((o) => o.status !== 'received' && o.status !== 'cancelled');
    safeStorage.setItem('orders', JSON.stringify(filtered));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:orders-updated'));
    } catch {
      // ignore
  ...
  getRevenue: () => {
    const revenue = parseJSON(safeStorage.getItem('revenue'), {
      ...
      lastYearlyStart: new Date(new Date().getFullYear(), 0, 1, 0, 0, 0, 0).toISOString(),
      dailyHistory: [],
      weeklyHistory: [],
      monthlyHistory: [],
      yearlyHistory: [],
  ...
  addRevenue: (amount: number) => {
    ...
    normalized.yearly = (normalized.yearly ?? 0) + amount;
  ...
  clearRevenueHistoryScope: (scope: 'daily' | 'weekly' | 'monthly' | 'yearly' | 'all') => {
    const revenue = storage.getRevenue();
    if (scope === 'daily' || scope === 'all') revenue.dailyHistory = [];
    if (scope === 'weekly' || scope === 'all') revenue.weeklyHistory = [];
    if (scope === 'monthly' || scope === 'all') revenue.monthlyHistory = [];
    if (scope === 'yearly' || scope === 'all') revenue.yearlyHistory = [];
    safeStorage.setItem('revenue', JSON.stringify(revenue));
  },
  ...
  // Staff -> Admin messaging (local)
  getStaffSalesReports: (): StaffSalesReport[] => {
    return parseJSON(safeStorage.getItem('staffSalesReports'), []);
  },
  addStaffSalesReport: (report: StaffSalesReport) => {
    const list = storage.getStaffSalesReports();
    list.push(report);
    safeStorage.setItem('staffSalesReports', JSON.stringify(list));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:reports-updated'));
    } catch {
      // ignore
    }
  },
  markStaffSalesReportRead: (id: string) => {
    const list = storage.getStaffSalesReports();
  ...
  deleteStaffSalesReport: (id: string) => {
    const list = storage.getStaffSalesReports().filter((r) => r.id !== id);
    safeStorage.setItem('staffSalesReports', JSON.stringify(list));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:reports-updated'));
    } catch {
  ...
  getStaffRestockAlerts: (): StaffRestockAlert[] => {
    return parseJSON(safeStorage.getItem('staffRestockAlerts'), []);
  },
  addStaffRestockAlert: (alert: StaffRestockAlert) => {
    const list = storage.getStaffRestockAlerts();
    list.push(alert);
    safeStorage.setItem('staffRestockAlerts', JSON.stringify(list));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:reports-updated'));
    } catch {
      // ignore
  ...
  markStaffRestockAlertRead: (id: string) => {
    const list = storage.getStaffRestockAlerts();
    const idx = list.findIndex((r) => r.id === id);
    if (idx >= 0) {
      list[idx] = { ...list[idx], status: 'read' };
  ...
  deleteStaffRestockAlert: (id: string) => {
    const list = storage.getStaffRestockAlerts().filter((r) => r.id !== id);
    safeStorage.setItem('staffRestockAlerts', JSON.stringify(list));
    try {
      window.dispatchEvent(new CustomEvent('vglecheflan:reports-updated'));
    } catch {
